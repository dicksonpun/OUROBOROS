...
namespace PANDA.ViewModel
{
    public class ClearcaseManagerViewModel : ViewModel
    {
        ...
        public async Task UpdateViewListPeriodically(TimeSpan interval, CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                // Asynchronously wait to enter the Semaphore. If no-one has been granted access to the Semaphore, code execution will proceed, otherwise this thread waits here until the m_mutex is released 
                ...
                {
                    // When the task is ready, release the m_mutex. It is vital to ALWAYS release the m_mutex when we are ready, or else we will end up with a Semaphore that is forever locked.
